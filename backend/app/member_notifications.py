...
async def notify_comunicazione_recipients(db, comm: dict) -> int:
    ...
    for member in destinatari:
        ...
        if await _send_to_member(member, subject, html):
            sent += 1
    if sent:
        logger.info("Notifiche comunicazione inviate: %s (comm=%s)", sent, comm.get("id"))
    return sent


async def notify_direct_message(db, recipient_id: str, sender_name: str, preview: str) -> bool:
...
async def notify_group_message(
    ...
    group = await db.chat_gruppi.find_one({"id": group_id}, {"_id": 0, "memberIds": 1, "nome": 1})
    if not group:
        return 0
    link = f"{PORTAL_BASE_URL}/area-associati/messaggi"
    sent = 0
    ...
    async def _run():
        try:
            if kind == "direct" and recipient_id:
                ...
                await notify_group_message(db, group_id, sender_id, sender_name, group_name, preview)
        except Exception:
