...
async def _presenza_map(db, event_id: str) -> dict[str, str]:
    rows = await db.presenze_evento.find({"eventId": event_id}, {"_id": 0}).to_list(
        5000
    )
...
async def _member_season_presenze_stats(
    db, member_id: str, season: str | None = None
) -> dict:
...
async def _upsert_presenza(
    db, event_id: str, member_id: str, stato: str, updated_by: str
) -> None:
...
async def portal_update_me(payload: ProfileUpdate, auth=Depends(require_member)):
    ...
    if (payload.emailNotifyComunicazioni or payload.emailNotifyMessages) and not (
        m.get("email") or ""
    ).strip():
        raise HTTPException(
            status_code=400,
    ...
    if payload.newPassword:
        if not payload.password or not verify_password(
            payload.password, m.get("passwordHash", "")
        ):
...
async def portal_dashboard(auth=Depends(require_member)):
    ...
    events = await _portal_events_for_member(
        db, mid, from_date=today, current_season=True, limit=1
    )
    pres_map = {}
    for ev in events:
        pres_map[ev["id"]] = (await _presenza_map(db, ev["id"])).get(
            mid, "NON_RISPOSTO"
        )
...
async def portal_calendario(auth=Depends(require_member)):
    ...
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    events = await _portal_events_for_member(
        db, mid, from_date=today, current_season=True
    )
    out = []
    for ev in events:
...
async def portal_set_presenza(
    event_id: str, body: SelfPresenzaBody, auth=Depends(require_member)
):
    ...
    existing_stato = _normalize_presenza_stato(
        existing.get("stato") if existing else None
    )
    if existing_stato in ("PRESENTE", "ASSENTE"):
        raise HTTPException(
            status_code=409, detail="Presenza già confermata e non modificabile"
        )
...
async def portal_storico(season: Optional[str] = None, auth=Depends(require_member)):
    ...
    all_items = (
        await db.designations.find(q, {"_id": 0}).sort("matchDate", -1).to_list(1000)
    )
    ...
    if season_clause:
        start, end = (
            season_clause["matchDate"]["$gte"],
            season_clause["matchDate"]["$lte"],
        )
...
async def portal_utility(auth=Depends(require_member)):
    ...
    links = []
    items = (
        await db.utility_items.find({"section": "link_utili"}, {"_id": 0})
        .sort("sortOrder", 1)
        .to_list(500)
    )
    for item in items:
...
async def portal_documenti(
    category: Optional[str] = None, auth=Depends(require_member)
):
    db = get_db()
    q = {}
    if category:
    ...
    mid = auth["memberId"]
    favs = await db.preferiti.find(
        {"memberId": mid, "tipo": "DOCUMENTO"}, {"_id": 0}
    ).to_list(500)
...
async def portal_comunicazioni(auth=Depends(require_member)):
    ...
    items = (
        await db.comunicazioni_interne.find(q, {"_id": 0})
        .sort("createdAt", -1)
        .to_list(200)
    )
    out = []
...
async def portal_media(auth=Depends(require_member)):
    ...
    items = (
        ...
        .sort([("sortOrder", 1), ("photoDate", -1), ("createdAt", -1)])
        .to_list(500)
    )
    for item in items:
...
async def portal_elimina_conversazione(chat_id: str, auth=Depends(require_member)):
    return await delete_conversation_for_member(
        get_db(), chat_id, auth["memberId"], _now()
    )
...
async def portal_upload_allegato(
    file: UploadFile = File(...), auth=Depends(require_member)
):
    allowed = {
        ".jpg",
        ".jpeg",
    ...
    tipo = (
        ...
        or ext in {".jpg", ".jpeg", ".png", ".webp", ".gif"}
        else "file"
    )
    return {
...
async def portal_preferiti(auth=Depends(require_member)):
    db = get_db()
    return await db.preferiti.find({"memberId": auth["memberId"]}, {"_id": 0}).to_list(
        500
    )
...
async def admin_presenze_evento_update(
    event_id: str, updates: List[PresenzaEventoUpdate], admin=Depends(require_admin)
):
    db = get_db()
...
async def admin_crea_comunicazione(
    payload: ComunicazioneInternaCreate, admin=Depends(require_admin)
):
    db = get_db()
    ...
    settings = (
        await db.site_settings.find_one({"id": "site-settings"}, {"_id": 0}) or {}
    )
    doc = {
        "id": str(uuid.uuid4()),
        "title": title,
