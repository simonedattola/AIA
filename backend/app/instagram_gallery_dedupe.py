...
from .instagram_gallery import instagram_shortcode_from_url, normalize_instagram_permalink


async def dedupe_instagram_gallery(db) -> int:
    items = await db.gallery_images.find({"source": "instagram"}, {"_id": 0}).to_list(5000)
    ...
    for code, group in by_code.items():
        if len(group) < 2:
            ...
            continue
        group.sort(key=lambda x: (0 if "/p/" in (x.get("sourceUrl") or "") and x["sourceUrl"].count("/") <= 5 else 1, x.get("createdAt") or ""))
        keep = group[0]
        canon = normalize_instagram_permalink(code)
        await db.gallery_images.update_one(
            {"id": keep["id"]},
