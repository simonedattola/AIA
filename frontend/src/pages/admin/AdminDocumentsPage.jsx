  ...
  return (
    <div data-testid="admin-documents">
      ...
      <AdminTableWrap>
          ...
          <tbody>
            {items.map((d) => (
              <tr key={d.id} className="border-t border-slate-100 hover:bg-slate-50/80">
                <td className="px-4 py-3 font-medium text-navy-700">{d.title}</td>
                <td className="px-4 py-3">
                  <AdminBadge variant="info">{d.category}</AdminBadge>
                </td>
                <td className="px-4 py-3 text-xs text-slate-500 truncate max-w-xs">
                  <a href={mediaUrl(d.fileUrl)} target="_blank" rel="noopener noreferrer" className="hover:text-navy-600">
                    {d.fileUrl}
                  </a>
                </td>
                <td className="px-4 py-3 text-right whitespace-nowrap">
                  ...
                  <button type="button" onClick={() => remove(d.id, d.title)} className="p-2 text-red-600 hover:bg-red-50 rounded ml-1">
        ...
        {items.length === 0 && <AdminEmptyState icon={SITE_ICONS.documents} title="Nessun documento." />}
      </AdminTableWrap>
    </div>
  );
}
