...
- Scheduler: `designations_scheduler.py` (default every 6h). Loop is resilient (errors do not kill the task). On Railway Free Serverless, overdue sync must be **awaited** (not fire-and-forget): `/api/health` awaits by default (`DESIGNATIONS_SYNC_AWAIT_ON_HEALTH=true`); prefer cron on `/api/cron/designations-sync`. Stale in-memory locks auto-clear after `DESIGNATIONS_SYNC_STALE_SEC` (default 720s).
- Manual: `POST /api/admin/designations/sync-aia` starts a **background** job (poll `GET /api/admin/designations/sync-status`); use `?force=true` if stuck. Do not await the full AIA crawl on that POST (Vercel proxy timeout).
