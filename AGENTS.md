...
- Scheduler: `designations_scheduler.py` (default every 6h). Loop is resilient (errors do not kill the task). `/api/health` (Railway) calls overdue watchdog + restarts a dead scheduler task.
- Manual: `POST /api/admin/designations/sync-aia` starts a **background** job (poll `GET /api/admin/designations/sync-status`); do not await the full AIA crawl on that POST (Vercel proxy timeout).
